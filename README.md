...
- **盤點作業**（手機優先）：頂部搜尋、狀態篩選、大數字輸入、**Enter 跳下一筆**、**自動儲存**；每筆顯示 編號 · 類別 · **規格**；四種狀態 **未盤 / 已盤 / 差異 / 已覆核**。
- **多人同時盤點防覆寫**：每筆明細有 version，採**樂觀鎖**；他人已更新時回 409 並提示，可選擇覆蓋。
- **差異覆核 + 報表匯出**：勾選品項標記已覆核；管理員/覆核員可**結案**。可**匯出差異報表（Excel / CSV）**，可選範圍（僅差異／全部已盤）與排序（依類別／依差異大小），含盤盈盤虧摘要與結案後調整紀錄。
...
- 來源產品：`SOURCE_SHEET_ID` 的工作表 `產品名單`（欄位：商品編號、商品名稱、類別、**規格**）。四欄都會匯入產品主檔並帶入盤點明細快照。
